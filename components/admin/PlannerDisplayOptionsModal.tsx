...
export const PlannerDisplayOptionsModal: React.FC<PlannerDisplayOptionsModalProps> = ({
  isOpen,
  onClose,
  onApply,
  employees,
  ...
  const handleSelectByEmploymentType = (type: EmploymentType) => {
    ...
    const newIds = employees
      .filter(emp => {
        const contract = getContractDetailsForDate(emp, today);
